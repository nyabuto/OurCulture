...
public class DropDownListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(AddTranslator.checkSelected[position])
	...
	private void setText(int position1){
		if (!AddTranslator.checkSelected[position1]) {
			AddTranslator.checkSelected[position1] = true;
			selectedCount++;
		} else {
			AddTranslator.checkSelected[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < AddTranslator.checkSelected.length; i++) {
				if (AddTranslator.checkSelected[i] == true) {
